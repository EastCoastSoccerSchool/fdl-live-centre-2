...
const styles = `
  * { box-sizing: border-box; }
  ...
  .mob-standings th:nth-child(2) { text-align: left; padding-left: 10px; }
  .mob-standings td {
    padding: 9px 6px;
    text-align: center;
  ...
  .mob-standings td:nth-child(2) { text-align: left; padding-left: 10px; font-weight: 600; }
  .mob-standings td:last-child { font-weight: 800; color: #111827; }
  ...
  .mob-finals-card .final-teams {
    font-size: 15px;
    font-weight: 700;
    color: #111827;
  }
  .mob-finals-card .final-tbc {
    font-size: 13px;
    color: #9ca3af;
  }

  /* ── Responsive: hide/show sections ── */
  ...
  @media (max-width: 768px) {
    ...
    .mob-age-row  { display: flex !important; }
...
export default function TournamentPanel({ mode = "public" }) {
  ...
  const [mobTab, setMobTab] = useState("fixtures"); // "fixtures" | "tables" | "finals"
  ...
  // Mobile fixtures: filtered by age tab
  const mobileFixtures = useMemo(() => {
    const filtered = fixtures.filter((f) => mobAge === "All" || f.age === mobAge);
    return timeOrder.map((time) => ({
      time,
      fixtures: filtered
  ...
  // Desktop standings card (unchanged)
  ...
  // Mobile standings card — compact, no overflow
  const mobStandingsCard = (title, rows) => (
    <div className="mob-standings">
      ...
      <table>
        <thead>
          <tr>
            <th>#</th><th style={{ textAlign: "left", paddingLeft: 10 }}>Team</th>
        ...
        <tbody>
          {rows.map((row, idx) => (
            <tr key={row.team}>
              <td>{idx + 1}</td>
              <td style={{ textAlign: "left", paddingLeft: 10, fontWeight: 600 }}>{row.team}</td>
              <td>{row.p}</td><td>{row.w}</td><td>{row.d}</td><td>{row.l}</td>
              <td>{row.gd}</td>
              <td style={{ fontWeight: 800 }}>{row.pts}</td>
            </tr>
          ))}
        </tbody>
      </table>
  ...
  return (
    <div className="page" style={{ overflowX: "hidden" }}>
      ...
      {/* ── MOBILE AGE FILTER (fixtures tab only) ── */}
      {mobTab === "fixtures" && (
        <div className="mob-age-row mobile-only">
          {AGE_OPTIONS.map((age) => (
            <button key={age} className={mobAge === age ? "active" : ""} onClick={() => setMobAge(age)}>
              {age}
            </button>
          ))}
        </div>
      ...
      <div className="mobile-only mob-content">
        ...
        {mobTab === "fixtures" && (
          <div>
            {mobileFixtures.length === 0 && (
              <p style={{ color: "#9ca3af", textAlign: "center", marginTop: 40 }}>No fixtures found.</p>
            )}
            {mobileFixtures.map(({ time, fixtures: atTime }) => (
              <div key={time}>
                ...
                {atTime.map((f) => {
                  ...
                  return (
                    <div key={f.id} className="mob-fixture-card">
        ...
        {mobTab === "tables" && (
          <div>
            ...
            <div className="mob-age-row" style={{ display: "flex", marginBottom: 12 }}>
              {AGE_OPTIONS.map((age) => (
                <button key={age} className={mobAge === age ? "active" : ""} onClick={() => setMobAge(age)}
                  style={{ flex: "0 0 auto", padding: "6px 16px", borderRadius: 99, border: "1.5px solid #d1d5db", background: mobAge === age ? "#1d4ed8" : "#fff", color: mobAge === age ? "#fff" : "#374151", fontWeight: 600, fontSize: 13, cursor: "pointer", marginRight: 8 }}>
                  {age}
                </button>
              ))}
            </div>
